...
from app.api.service.ground_water_management.gwpz_operation import GWPZone,GWPumpingMapper, MARRasterDetails,MARSuitabilityMapper
...
async def gwz_raster_dislay(db:db_dependency,payload:category_raster,):
    return await GWPZone().get_visual_raster(db,payload.clip,payload.place,payload.layer_name)
...
async def gwz_raster_operation(db:db_dependency,payload: STPCategory,):
    return await GWPZone().create_gwpz_map(db,payload)
...
async def gwpl_raster_dislay(db:db_dependency,payload:category_raster,):
    return await GWPumpingMapper().get_visual_raster(db,payload.clip)
...
async def gwpl_raster_operation(db:db_dependency,payload: STPsuitabilityInput,):
    return await GWPumpingMapper().create_gwpz_map(db,payload)
...
async def gwpl_find_score(db:db_dependency,payload:GWPL_Table_input,):
    return await GWPumpingMapper().gwpl_table(db,payload.raster_name,payload.location,payload.village_layer)
...
@validate
async def gwz_raster_dislay(db:db_dependency,payload:category_raster,):
    return await MARSuitabilityMapper().get_visual_raster(db,payload.clip,payload.place,payload.layer_name)
...
async def stp_classify(db:db_dependency,payload:STPsuitabilityInput):
    return await MARSuitabilityMapper().create_suitability_map(db,payload)
