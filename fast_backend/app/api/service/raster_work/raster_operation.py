...
class RasterOperation:
    ...
    def _format_resolution(self, res_x: float, res_y: float, unit: str) -> Dict[str, Any]:
        ...
        def _humanize(value: float, unit: str):
            """Convert to cm if sub-metre, keep metres otherwise; leave degrees alone."""
            if unit == "degree":
            ...
            if "foot" in unit or "feet" in unit:
                metres = value * 0.3048
                return {"value": round(metres, 4), "unit": "m", "original_value": round(value, 4), "original_unit": unit}
    ...
    def _basic_info(self, src, file_path: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            "nodata": self._safe_float(src.nodata),  # ← was missing _safe_float
        }
    ...
    def _sampled_stats(self, src, band_index: int, sample_size: int = 1024):
        height = min(sample_size, src.height)
        width = min(sample_size, src.width)
        ...
        if valid.size == 0:
            return {"min": None, "max": None, "mean": None, "std": None, "sample_note": "no valid pixels"}
        ...
        return {
            ...
            "sample_note": f"sampled from top-left {width}×{height} pixels",
        }
    ...
    def save_upload(self, file: UploadFile) -> str:
        ...
        if file_size > 200 * 1024 * 1024:
            raise ValueError("File size limit exceeded than 200MB")
    ...
    def get_raster_info(self, file_id: str, compute_stats=True) -> Dict[str, Any]:
