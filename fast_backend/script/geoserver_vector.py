...
groundwater_location = os.path.join(BASE_DIRR,  "csv_file_stp", "gwpl_visual.csv")
...
try:
    ...
    create_workspace("dss_visualization")
    create_vector_stores("dss_vector","stp_vector_store")
    upload_shapefile("dss_vector","stp_vector_store",state_zip,"STP_state_layers")
    upload_shapefile("dss_vector","stp_vector_store",district_zip,"STP_district_layers")
    upload_shapefile("dss_vector","stp_vector_store",subdistrict_zip,"STP_subdistrict_layers")
    upload_shapefile("dss_vector","stp_vector_store",villages_zip,"STP_villages_layers")
    upload_shapefile("dss_vector","stp_vector_store",river_zip,"Rivers")
    upload_shapefile("dss_vector","stp_vector_store",stretch_zip,"Stretches")
    upload_shapefile("dss_vector","stp_vector_store",drain_zip,"Drain")
    upload_shapefile("dss_vector","stp_vector_store",boundry_zip,"Boundary")
    upload_shapefile("dss_vector","stp_vector_store",town_zip,"Town")
    upload_shapefile("dss_vector","stp_vector_store",drain_buffer_zip,"Drain_Suitability")
    ...
    for i in all_layers.iterrows():
        visual_raster.publish_raster(workspace_name="dss_visualization", store_name=uuid.uuid4().hex, raster_path=i[1]["file_path"],layer_name=i[1]["layer_name"])
        visual_raster.apply_sld_to_layer(workspace_name="dss_visualization", layer_name=i[1]["layer_name"], sld_content=i[1]["sld_path"], sld_name=uuid.uuid4().hex)
