...
interface AdminMapHeaderControlsProps {
  ...
  hasPinnedPoint: boolean;
  onToggleVectorInteraction: () => void;
  ...
  return (
    <div className="absolute left-1/2 top-3 z-10 flex w-[calc(100%-1rem)] max-w-max -translate-x-1/2 items-center justify-between gap-2 rounded-2xl bg-white/20 px-2 py-1.5 shadow-xl backdrop-blur-md sm:w-auto sm:gap-4 sm:px-3">
      ...
      <div className="flex flex-1 justify-center space-x-1 sm:flex-none sm:space-x-2">
        {showAnalysisToggle && (
          <button
            onClick={onToggleVectorInteraction}
            className={`relative rounded-full p-2 transition-all duration-200 hover:scale-110 sm:p-2.5 ${
              vectorInteractionEnabled
          ...
          >
            {vectorInteractionEnabled && hasPinnedPoint && (
            ...
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                ...
                strokeLinejoin="round"
                strokeWidth={2}
                d={vectorInteractionEnabled ? ANALYSIS_ON_ICON : ANALYSIS_OFF_ICON}
              />
            </svg>
          </button>
        )}
