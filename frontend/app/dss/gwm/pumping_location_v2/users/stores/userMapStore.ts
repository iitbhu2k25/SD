...
import { ClipRasters, DRAIN_LAYER_NAMES } from "@/interface/raster_context";
import { runUserPumpingAnalysis } from "../../services/gwmPumpingApi";
...
type UserMapSet = (
  partial:
    | Partial<UserMapStore>
    | ((state: UserMapStore) => Partial<UserMapStore>),
) => void;
...
export const useUserMapStore = create<UserMapStore>((set) => ({
  ...
  runAnalysis: async () => {
    ...
    const { resultLayer, displayRaster, setDisplayRaster } = useUserRiverStore.getState();

    const selectedCategories = [...selectedCondition, ...selectedConstraint];
    if (selectedCategories.length === 0 || !resultLayer) {
      return;
    }

    set({
      pumpingOperation: true,
      rasterLoading: true,
      error: null,
      wmsDebugInfo: null,
    });
    setPumpingProcess(true);

    try {
      const result = await runUserPumpingAnalysis({
        data: selectedCategories,
        village_layer: resultLayer,
      });

      const pumpingRaster: ClipRasters = {
      ...
      const existingIndex = displayRaster.findIndex(
        (item) => item.file_name === pumpingRaster.file_name,
      );
      const nextRaster =
        existingIndex === -1
          ? [...displayRaster, pumpingRaster]
          : displayRaster.map((item, index) =>
              index === existingIndex ? pumpingRaster : item,
            );

      setDisplayRaster(nextRaster);
      set({
        rasterLayerInfo: pumpingRaster,
        selectedradioLayer: pumpingRaster.file_name,
        showLegend: true,
      });
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Error communicating with pumping service";
      set({ error: message });
    } finally {
      set({
        pumpingOperation: false,
