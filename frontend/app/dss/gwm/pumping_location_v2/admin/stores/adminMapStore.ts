...
import { runAdminPumpingAnalysis } from "../../services/gwmPumpingApi";
...
async function runAdminAnalysis(set: AdminMapSet) {
  const { selectedCondition, selectedConstraint, setStpProcess } =
  ...
  if (selectedCategories.length === 0 || selectedVillages.length === 0 || !resultLayer) {
    return;
  }
  ...
  try {
    const result = await runAdminPumpingAnalysis({
