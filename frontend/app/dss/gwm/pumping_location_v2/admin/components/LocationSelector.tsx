...
import { villages } from "@/interface/raster_context";
...
interface LocationSelectorProps {
  onConfirm?: (selectedData: { villages: villages[] }) => void;
}
...
export default function LocationSelector({ onConfirm }: LocationSelectorProps) {
  ...
  const villagesList = useAdminLocationStore((state) => state.villages);
  ...
  const handleDistrictsChange = (selectedIds: number[]) => {
    if (!selectionsLocked) {
      void setSelectedDistricts(selectedIds);
    }
  };

  const handleSubDistrictsChange = (selectedIds: number[]) => {
    if (!selectionsLocked) {
      void setSelectedSubDistricts(selectedIds);
    }
  };
    ...
    >
      <div className="mb-3 grid grid-cols-1 gap-3 sm:mb-4 sm:gap-4">
        <SingleSelect
          ...
          onValueChange={(id) => void handleStateChange(id === null ? Number.NaN : Number(id))}
          label="State"
