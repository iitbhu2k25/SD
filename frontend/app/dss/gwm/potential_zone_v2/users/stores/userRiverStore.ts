...
import {
  fetchPotentialCatchments,
  fetchUserDisplayRaster,
  fetchUserRiverReferenceData,
} from "../../services/gwzPotentialApi";
import {
...
export const useUserRiverStore = create<UserRiverState>((set, get) => ({
  ...
  setShowCatchment: async (value) => {
    ...
    set({ isLoading: true, error: null });
    try {
      // Uses the shared STP endpoint: /stp_operation/get_priority_cachement
      const response = await fetchPotentialCatchments(selectedDrains);
