...
import { runAdminPotentialAnalysis } from "../../services/gwzPotentialApi";
...
async function runAdminAnalysis(set: AdminMapSet) {
  ...
  try {
    const result = await runAdminPotentialAnalysis({
    ...
    const potentialRaster: ClipRasters = {
    ...
    const existingIndex = displayRaster.findIndex(
      (item) => item.file_name === potentialRaster.file_name,
    );
    const nextRaster =
      existingIndex === -1
        ? [...displayRaster, potentialRaster]
        : displayRaster.map((item, index) =>
            index === existingIndex ? potentialRaster : item,
          );

    setTableData(result.csv_details);
    setDisplayRaster(nextRaster);
    set({
      rasterLayerInfo: potentialRaster,
      selectedradioLayer: potentialRaster.file_name,
      showLegend: true,
    });
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : "Error communicating with GWZ service";
    set({ error: message });
    setTableData([]);
  } finally {
    set({
