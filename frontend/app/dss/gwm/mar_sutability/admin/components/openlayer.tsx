...
const Mapping: React.FC = () => {
  ...
  const handleVectorLayer = (layer: string | null, type: 'primary' | 'secondary' | 'result') => {
    ...
    const wfsUrl = `/geoserver/api/wfs?service=WFS&version=1.1.0&request=GetFeature&typeName=${defaultWorkspace}:${layer}&outputFormat=application/json&srsname=EPSG:3857${
  ...
  useEffect(() => {
    ...
    if (stpOperation) {
      const performSTP = async () => {
        try {
          const resp = await fetch("/api/gwz_operation/mar_sutability", {
            method: "POST",
          ...
          if (result && result.status === "success") {
            ...
            const index = newData.findIndex(item => item.file_name === "mar_sutability");
            if (index !== -1) {
