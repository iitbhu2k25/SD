        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              ...
              value={yearStart}
              onChange={(e) => { setYearStart(e.target.value); if (yearEnd && parseInt(e.target.value) >= parseInt(yearEnd)) setYearEnd(''); }}
              disabled={isLoading || yearsToShow.length === 0}
            >
              <option value="">{yearsToShow.length === 0 ? "No years" : "From year"}</option>
              {yearsToShow.map(year => <option key={year} value={year}>{year}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
            ...
            >
              <option value="">{!yearStart ? "Select from first" : "To year"}</option>
              {yearsToShow.filter(year => !yearStart || parseInt(year) > parseInt(yearStart)).map(year => <option key={year} value={year}>{year}</option>)}
            </select>
          </div>
        </div>
