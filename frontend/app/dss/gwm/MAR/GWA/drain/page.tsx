...
import { MapProvider } from "@/contexts/groundwater_assessment/drain/MapContext";
...
export interface DrainGWAProps {
  activeStep: number;
...
function DrainRightPanel({
  ...
  const handleComputeWater = () => {
    ...
    if (stateCode) params.append("state", stateCode.toString());
    if (districtCodes.length > 0) params.append("districts", districtCodes.join(","));
    if (subdistrictCodes.length > 0) params.append("subdistricts", subdistrictCodes.join(","));
  ...
  return (
    <>
      ...
      >
        {isOpen && (
          <div onMouseDown={handleResizeMouseDown} className="group absolute inset-y-0 left-0 z-10 hidden w-2 cursor-col-resize items-center justify-center transition-colors hover:bg-emerald-400/20 lg:flex">
        ...
        {isOpen && (
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-stone-200 bg-[#f7f4ef]/88 px-3 py-2.5 backdrop-blur-sm">
              <h2 className="text-[14px] font-semibold tracking-[0.02em] text-slate-800">Analysis Output</h2>
              <button onClick={onClose} className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-stone-200 bg-white/80 text-slate-500 transition hover:border-rose-200 hover:text-rose-600" title="Close panel">
            ...
            <div className="flex-1 overflow-y-auto p-3 pt-6 space-y-3">
              ...
              {activeStep === 2 && <GroundwaterTrend activeTab="groundwater-trend" step={activeStep} />}
              {activeStep === 3 && <GSR step={activeStep} />}
...
function DrainGWALayout({ activeStep, enableGroundwaterDepth, enableTimeseriesAnalysis, onSelectionsLockedChange, onModeChange, currentMode, optionalStepsBar, navigationProps }: DrainGWAProps) {
  ...
  useEffect(() => {
    ...
    const handle = (e: MediaQueryListEvent) => { setIsMobile(!e.matches); if (!e.matches) setIsPanelOpen(false); };
...
// ─── Provider wrapper ─────────────────────────────────────────────────────────
export default function GroundwaterAssessmentDrain(props: DrainGWAProps) {
  return (
    <LocationProvider>
      <AdminUnitsProvider>
        <WellProvider>
