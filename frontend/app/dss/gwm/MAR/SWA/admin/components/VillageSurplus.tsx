// app/(routes)/surfacewater_assessment/village/VillageSurplus.tsx
'use client';
...
export default function VillageSurplus() {
  ...
  const { loading, error, items, hasData, fetchData, lastFetchedSubdistricts } = useVillageSurplusContext();
  ...
  const currentVillageData = useMemo(() => {
    ...
    return items.find(s => String(s.vlcode) === String(selectedVillage));
  ...
  const handleVillageChange = useCallback((newVillage: string) => {
    console.log('Changing village from', selectedVillage, 'to', newVillage);
  ...
  const downloadServerPng = useCallback(() => {
    if (!currentVillageData?.image_base64) {
      console.warn('No server PNG available for this village');
      return;
    }
    const safeVillage = (currentVillageData.village || 'Village').replace(/[^a-z0-9-_]+/gi, '_');
    const a = document.createElement('a');
    a.href = `data:image/png;base64,${currentVillageData.image_base64}`;
      ...
      {!isFullscreen && (
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Village Surplus (Q25)</h2>
