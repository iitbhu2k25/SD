// app/(routes)/surfacewater_assessment/village/StreamFlow.tsx
'use client';
...
export default function StreamFlow() {
  ...
  const { loading, error, series, hasData, fetchData, lastFetchedSubdistricts } = useStreamFlowContext();
  ...
  const downloadServerPng = useCallback(() => {
    if (!selectedVillage) return;
    const item = series.find(s => s.vlcode === selectedVillage);
    if (!item?.imageBase64) {
      console.warn('No server PNG available for this village');
      return;
    }
    const safeVillage = (item.village || 'Village').replace(/[^a-z0-9-_]+/gi, '_');
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                {series.length} village{series.length !== 1 ? 's' : ''}
      ...
      >
        <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Village selector handled below when not fullscreen as well */}
          </div>
