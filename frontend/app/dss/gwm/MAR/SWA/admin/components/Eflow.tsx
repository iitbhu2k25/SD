...
// Helper function to get original column name from display name
...
export default function Eflow() {
  ...
  const currentVillage = useMemo(() => {
    ...
    return items.find(s => s.vlcode === selectedVillage);
  }, [items, selectedVillage]);
  ...
  const handleVillageChange = useCallback((newVlcode: number) => {
    setSelectedVillage(newVlcode);
    setChartKey(prev => prev + 1);
  }, []);
  ...
  // UPDATED: CSV download with proper column mapping
  const handleDownloadCSV = useCallback(() => {
    ...
    const rows = mergedItems.map(r => {
      ...
      // Get stress values using original column names
  ...
  // PNG download
  const handleDownloadPNG = useCallback((vlcode: number, method: MethodKey) => {
    const rec = items.find(r => r.vlcode === vlcode);
    const b64 = rec?.curves?.[method]?.image_base64;
    if (!b64) return;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Method Comparison Table with Stress Data */}
      {!isFullscreen && (
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <table className="min-w-full text-sm">
              <thead className="sticky top-0 bg-white">
                <tr className="text-left border-b">
                  ...
                  {/* Stress data columns with custom display names */}
              ...
              <tbody>
                {mergedItems.map(r => (
                  <tr key={r.vlcode} className="border-b hover:bg-gray-50">
                    ...
                    {/* Stress data columns with proper mapping */}
                    {hasStressData && stressColumns.map(displayCol => {
                      const originalCol = getOriginalColumnName(displayCol);
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <YAxis
                  label={{ value: 'Flow (cms)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle' } }}
                />
