...
const OPERATIONS: Operation[] = [
  { id:'intersection',         icon:'fg-intersection',   name:'Intersection',       category:'Overlay',   minFiles:2, layerLabels:['Layer A','Layer B'],           description:'Find the overlap area shared by all selected layers' },
  { id:'union',                icon:'fg-union',          name:'Union',              category:'Overlay',   minFiles:2, layerLabels:['Layer A','Layer B'],           description:'Merge all selected layers into a single combined layer' },
  { id:'difference',           icon:'fg-difference',     name:'Difference',         category:'Overlay',   minFiles:2, layerLabels:['Input Layer','Erase Layer'],   description:'Subtract the erase layer from the input layer' },
  { id:'symmetric_difference', icon:'fg-sym-difference', name:'Sym. Difference',    category:'Overlay',   minFiles:2, maxFiles:2, layerLabels:['Layer A','Layer B'],description:'Areas belonging to one layer but not both' },
  { id:'clip',                 icon:'fg-copy-poly',      name:'Clip',               category:'Overlay',   minFiles:2, maxFiles:2, layerLabels:['Input Layer','Clip Boundary'], description:'Clip the input layer to the shape of the boundary layer' },
  ...
  { id:'simplify',             icon:'fg-simplify',       name:'Simplify',           category:'Geometric', minFiles:1, layerLabels:['Input Layer'],                description:'Reduce vertex count while preserving topology',
    parameters:[{ name:'tolerance', label:'Tolerance', type:'number', default:0.001, required:true, placeholder:'0.001' }] },
  { id:'statistics',           icon:'fg-layer-stat',     name:'Statistics',         category:'Analysis',  minFiles:1, layerLabels:['Input Layer'],                description:'Compute area, length, perimeter or centroid metrics',
  ...
  { id:'spatial_join',         icon:'fg-layer-stack',    name:'Spatial Join',       category:'Analysis',  minFiles:2, maxFiles:2, layerLabels:['Target Layer','Join Layer'], description:'Attach attributes from Join Layer to Target by location',
    parameters:[
  ...
  { id:'nearest',              icon:'fg-location-arrow', name:'Nearest Neighbor',   category:'Analysis',  minFiles:2, maxFiles:2, layerLabels:['Source Layer','Target Layer'], description:'Find the closest feature in Target for each Source feature' },
  { id:'point_in_polygon',     icon:'fg-polygon-pt',     name:'Point in Polygon',   category:'Analysis',  minFiles:2, maxFiles:2, layerLabels:['Points Layer','Polygons Layer'], description:'Select points that fall inside the polygons layer' },
  { id:'area_comparison',      icon:'fg-measure-area',   name:'Area Comparison',    category:'Analysis',  minFiles:1, layerLabels:['Input Layer'],                description:'Rank features by area and compute percentage shares' },
  { id:'topology_check',       icon:'fg-map-edit',       name:'Topology Check',     category:'Analysis',  minFiles:1, layerLabels:['Input Layer'],                description:'Detect invalid geometries and attempt auto-repair' },
  ...
  { id:'voronoi',              icon:'fg-voronoi-map',    name:'Voronoi Diagram',    category:'Utility',   minFiles:1, layerLabels:['Points Layer'],               description:'Generate Voronoi tessellation from point features' },
  { id:'reproject',            icon:'fg-coord-system',   name:'Reproject',          category:'Utility',   minFiles:1, layerLabels:['Input Layer'],                description:'Transform layer to a different coordinate reference system',
    parameters:[{ name:'target_crs', label:'Target CRS', type:'text', default:'EPSG:4326', placeholder:'e.g. EPSG:3857' }] },
  { id:'snap_to_grid',         icon:'fg-snap',           name:'Snap to Grid',       category:'Utility',   minFiles:1, layerLabels:['Input Layer'],                description:'Snap coordinate vertices to a regular grid for precision control',
    parameters:[{ name:'grid_size', label:'Grid Size (degrees)', type:'number', default:0.0001, required:true }] },
  // ── New Geometric Operations ───────────────────────────────────────────────
  { id:'rotate',               icon:'fg-rotate',         name:'Rotate',             category:'Geometric', minFiles:1, layerLabels:['Input Layer'],                description:'Rotate each feature geometry by a given angle around its centroid',
    parameters:[{ name:'angle', label:'Angle (degrees)', type:'number', default:45, required:true }] },
  { id:'scale',                icon:'fg-scale',          name:'Scale',              category:'Geometric', minFiles:1, layerLabels:['Input Layer'],                description:'Scale each feature geometry by X and Y factors from its centroid',
    parameters:[
      { name:'xfact', label:'X Scale Factor', type:'number', default:1.5, required:true },
      { name:'yfact', label:'Y Scale Factor', type:'number', default:1.5, required:true },
    ] },
  { id:'translate',            icon:'fg-move',           name:'Translate',          category:'Geometric', minFiles:1, layerLabels:['Input Layer'],                description:'Move each feature by an offset in degrees (longitude, latitude)',
    parameters:[
      { name:'xoff', label:'X Offset (°lon)', type:'number', default:0.01, required:true },
      { name:'yoff', label:'Y Offset (°lat)', type:'number', default:0.01, required:true },
    ] },
  { id:'minimum_bounding_circle', icon:'fg-circle',      name:'Min. Bounding Circle', category:'Geometric', minFiles:1, layerLabels:['Input Layer'], description:'Compute the minimum enclosing circle for each feature' },
  { id:'polygon_to_line',      icon:'fg-polyline-pt',    name:'Polygon → Line',     category:'Geometric', minFiles:1, layerLabels:['Input Layer'],                description:'Extract polygon boundaries as linestring geometries' },
];

const CATS = [
  { id:'Overlay',   label:'Overlay',   icon:'fg-intersection', color:'#0ea5e9', bg:'#f0f9ff' },
  { id:'Geometric', label:'Geometric', icon:'fg-polygon-pt',   color:'#8b5cf6', bg:'#f5f3ff' },
  { id:'Analysis',  label:'Analysis',  icon:'fg-layer-stat',   color:'#10b981', bg:'#f0fdf4' },
  { id:'Utility',   label:'Utility',   icon:'fg-earth-gear',   color:'#f59e0b', bg:'#fffbeb' },
];
const getCat = (id: string) => CATS.find(c => c.id === id) ?? CATS[0];

/* ─── Spatial Queries catalogue ──────────────────────────────────────────── */
const QUERY_CATS = [
  { id:'Topological', label:'Topological', icon:'fg-intersection',  color:'#0f766e', bg:'#f0fdfa' },
  { id:'Distance',    label:'Distance',    icon:'fg-measure-line',  color:'#7c3aed', bg:'#faf5ff' },
  { id:'Aggregate',   label:'Aggregate',  icon:'fg-layer-stat',    color:'#b45309', bg:'#fffbeb' },
  { id:'Advanced',    label:'Advanced',    icon:'fg-map-stat',      color:'#dc2626', bg:'#fef2f2' },
];
const getQCat = (id: string) => QUERY_CATS.find(c => c.id === id) ?? QUERY_CATS[0];

const QUERIES: QueryDef[] = [
  // Topological
  { id:'intersects',        icon:'fg-intersection',   name:'Intersects',           category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features that intersect any target feature' },
  { id:'within',            icon:'fg-polygon',        name:'Within',               category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features completely inside any target feature' },
  { id:'contains',          icon:'fg-polygon-pt',     name:'Contains',             category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features that fully contain any target feature' },
  { id:'touches',           icon:'fg-polygon-hole',   name:'Touches',              category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features sharing a boundary with any target feature' },
  { id:'crosses',           icon:'fg-measure-line',   name:'Crosses',              category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source lines/polygons that cross any target feature' },
  { id:'overlaps',          icon:'fg-union',          name:'Overlaps',             category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features that partially overlap any target feature' },
  { id:'disjoint',          icon:'fg-difference',     name:'Disjoint',             category:'Topological', layerLabels:['Source Layer','Target Layer'],   description:'Select source features with NO spatial relation to target' },
  // Distance
  { id:'within_distance',     icon:'fg-measure-line',  name:'Within Distance',     category:'Distance', layerLabels:['Source Layer','Target Layer'],
  ...
  { id:'not_within_distance', icon:'fg-measure',       name:'Not Within Distance', category:'Distance', layerLabels:['Source Layer','Target Layer'],
    description:'Select source features farther than a given distance from all target features',
    parameters:[{ name:'distance_m', label:'Distance (meters)', type:'number', default:1000, required:true }] },
  { id:'distance_band',       icon:'fg-measure-area',  name:'Distance Band',       category:'Distance', layerLabels:['Source Layer','Target Layer'],
  ...
  { id:'nearest_n',           icon:'fg-location-arrow', name:'Nearest N Features', category:'Distance', layerLabels:['Source Layer','Target Layer'],
  ...
  // Advanced
  { id:'select_by_location', icon:'fg-search-feature', name:'Select by Location', category:'Advanced', layerLabels:['Source Layer','Target Layer'],
  ...
  { id:'relate',             icon:'fg-map-search',    name:'Spatial Relate (all)', category:'Advanced', layerLabels:['Source Layer','Target Layer'],
...
export default function SpatialAnalysisModal({
  isOpen, onOpenChange, managedLayers, showNotification, onAnalysisComplete, initialOperationId,
  ...
  useEffect(() => {
    if (!isOpen || !initialOperationId) return;
    const op = OPERATIONS.find(o => o.id === initialOperationId);
    if (op) {
      setMainTab('operations');
      setActiveCategory(op.category);
      selectOp(op);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, initialOperationId]);
          ...
          <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
            ...
            <div style={{ width:148, background:'#f8fafc', borderRight:'1px solid #e2e8f0',
              ...
              {CATS.map(c => {
                ...
                return (
                  ...
                  >
                    <i className={c.icon} style={{ fontSize:16, flexShrink:0 }} />{c.label}
                  </button>
                );
              })}
            ...
            <div style={{ width:210, borderRight:'1px solid #e2e8f0',
              ...
              <div style={{ padding:'12px 14px 8px', borderBottom:'1px solid #e2e8f0', background:cat.bg, flexShrink:0 }}>
                <div style={{ fontSize:11, fontWeight:800, color:cat.color, letterSpacing:1.2, textTransform:'uppercase', display:'flex', alignItems:'center', gap:5 }}>
                  <i className={cat.icon} style={{ fontSize:13 }} /> {cat.label}
                </div>
                ...
                {filteredOps.map(op => {
                  ...
                  return (
                    <button key={op.id} onClick={()=>selectOp(op)}
                      ...
                      onMouseLeave={e=>{ if(!active)(e.currentTarget as HTMLElement).style.background='transparent'; }}
                    >
                      <i className={op.icon} style={{ fontSize:18, flexShrink:0, width:24, textAlign:'center', color: active ? cat.color : '#94a3b8', lineHeight:1.2 }} />
                      <div>
                ...
                <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center',
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, width:'100%', maxWidth:380, marginTop:8 }}>
                    {CATS.map(c => (
                      ...
                      >
                        <i className={c.icon} style={{ fontSize:24, marginBottom:4, color:c.color, display:'block' }} />
                ...
                <div style={{ padding:'22px 26px', display:'flex', flexDirection:'column', gap:18, flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:14, padding:'16px 20px', background:'#fff',
                    ...
                    <div style={{ width:50, height:50, borderRadius:13, background:cat.bg,
                      ...
                      <i className={selectedOp.icon} style={{ fontSize:26, color:cat.color }} />
                    </div>
                    <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:10, paddingTop:4, marginTop:'auto' }}>
                    <button onClick={runOp} disabled={!opReady||opProcessing}
                      ...
                      {opProcessing ? '⏳  Processing…' : <><i className={selectedOp.icon} style={{ marginRight:6 }} />Execute {selectedOp.name}</>}
                    </button>
          ...
          <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
            ...
            <div style={{ width:210, borderRight:'1px solid #e2e8f0',
              ...
              <div style={{ padding:'12px 14px 8px', borderBottom:'1px solid #e2e8f0', background:qcat.bg, flexShrink:0 }}>
                <div style={{ fontSize:11, fontWeight:800, color:qcat.color, letterSpacing:1.2, textTransform:'uppercase', display:'flex', alignItems:'center', gap:5 }}>
                  <i className={qcat.icon} style={{ fontSize:13 }} /> {qcat.label}
                </div>
                ...
                {filteredQs.map(q => {
                  ...
                  return (
                    <button key={q.id} onClick={()=>selectQuery(q)}
                      ...
                      onMouseLeave={e=>{ if(!active)(e.currentTarget as HTMLElement).style.background='transparent'; }}
                    >
                      <i className={q.icon} style={{ fontSize:17, flexShrink:0, width:24, textAlign:'center', color: active ? qcat.color : '#94a3b8', lineHeight:1.2 }} />
                      <div>
                ...
                <div style={{ padding:'22px 26px', display:'flex', flexDirection:'column', gap:18, flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:14, padding:'16px 20px', background:'#fff',
                    ...
                    <div style={{ width:50, height:50, borderRadius:13, background:qcat.bg,
                      display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <i className={selectedQuery.icon} style={{ fontSize:26, color:qcat.color }} />
                    </div>
                    <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:10, paddingTop:4, marginTop:'auto' }}>
                    <button onClick={runQuery} disabled={!qReady||qProcessing}
                      ...
                      {qProcessing ? '⏳  Running Query…' : <><i className={selectedQuery.icon} style={{ marginRight:6 }} />Run: {selectedQuery.name}</>}
                    </button>
