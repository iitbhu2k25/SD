...
const FILE_CHUNK_SIZE = 1 * 1024 * 1024; // 1 MB per binary chunk
 ...
 *
 * Files larger than 1 MB are automatically split into 1 MB binary chunks and
...
/**
 * Fetch attribute/field names for a GeoJSON layer from the backend.
 *
 * For layers already loaded on the map prefer extracting properties
 * client-side (see SpatialAnalysisModal).  Use this when you only
 * have an ``upload_id`` (just-uploaded file not yet fully processed).
 */
export async function fetchLayerFields(geojson: any): Promise<string[]> {
  const r = await prepareGeoJSONField(geojson);
  const body = r.upload_id
    ? { upload_id: r.upload_id }
    : { geojson: r.direct ? JSON.parse(r.direct) : geojson };
  const res = await fetch(`${process.env.NEXT_PUBLIC_FAST_URL}/mapplot/fields`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
  if (!res.ok) throw new Error('Failed to fetch layer fields');
