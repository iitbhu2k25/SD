...
export async function fetchShapefileDirectory(): Promise<Record<string, string[]>> {
  const response = await fetch('/django/mapplot/shapefiles');
...
export async function fetchGeoJSON(category: string, subcategory: string): Promise<any> {
  const response = await fetch(
    `/django/mapplot/get_shapefile/?category=${encodeURIComponent(category)}&subcategory=${encodeURIComponent(subcategory)}`
  );
...
export async function uploadShapefile(
  ...
  const res = await fetch('/django/mapplot/upload-shapefile', {
    method: 'POST',
    body: form,
  });
...
export async function performSpatialAnalysis(
  ...
  const response = await fetch('/django/mapplot/spatial/process', { method: 'POST', body: formData });
  if (!response.ok) {
...
export async function getSpatialOperations(): Promise<any> {
  const response = await fetch('/django/mapplot/spatial/operations');
...
export async function exportMapPNG(geojson: any, width = 1920, height = 1080, dpi = 300): Promise<Blob> {
  const r = await fetch('/django/mapplot/export/png', {
...
export async function exportMapPDFServer(
  ...
  const r = await fetch('/django/mapplot/export/pdf', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
...
export async function geojsonToShapefile(geojson: any, filename = 'export'): Promise<Blob> {
  const r = await fetch('/django/mapplot/export/shapefile', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
