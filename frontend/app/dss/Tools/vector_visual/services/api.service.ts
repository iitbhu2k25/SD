...
export async function fetchShapefileDirectory(): Promise<Record<string, string[]>> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/shapefiles`);
...
export async function fetchGeoJSON(category: string, subcategory: string): Promise<any> {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/get_shapefile/?category=${encodeURIComponent(category)}&subcategory=${encodeURIComponent(subcategory)}`
  );
...
export async function uploadShapefile(
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/upload-shapefile`, {
    method: 'POST',
    body: form,
  });
...
export async function performSpatialAnalysis(
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/spatial/process`, { method: 'POST', body: formData });
  if (!response.ok) {
...
export async function getSpatialOperations(): Promise<any> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/spatial/operations`);
...
export async function exportMapPNG(geojson: any, width = 1920, height = 1080, dpi = 300): Promise<Blob> {
  const r = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/export/png`, {
...
export async function exportMapPDFServer(
  ...
  const r = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/export/pdf`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
...
export async function geojsonToShapefile(geojson: any, filename = 'export'): Promise<Blob> {
  const r = await fetch(`${process.env.NEXT_PUBLIC_DJANGO_URL}/mapplot/export/shapefile`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
