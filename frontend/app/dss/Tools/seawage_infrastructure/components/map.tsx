    ...
    return (
        <div className="h-full relative">
            <div className="h-full relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden shadow-2xl">
                ...
                {/* Click Feature Info Popup - Shows ALL properties */}
                <div
                    ref={popupRef}
                    className={`bg-white rounded-md shadow-2xl border border-gray-200 p-2 min-w-[220px] max-w-[320px] backdrop-blur-sm bg-white/95 ${featureInfo ? 'block' : 'hidden'}`}
                >
                    {featureInfo && (
                        <>
                                ...
                                <>
                                    ...
                                    <div className="mt-2 pt-1.5 border-t border-gray-200">
                                        <p className="text-[8px] text-gray-500 text-center">
                                            Showing all {Object.keys(featureInfo.properties).filter(k => k !== 'isEditable').length} properties
                                        </p>
                                    </div>
                                </>
                            )}
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                if (layer.id === 'basin') {
                                                                    ...
                                                                    setActiveLayerFid(layer.fid || null);
                                                                }
                                                            }}
                                                            className={`px-3 py-2 rounded-lg font-semibold text-xs transition-all duration-200 flex items-center gap-1 ${activeStyleEditor === (layer.id === 'basin' ? 'basin' : 'main') && activeLayerFid === (layer.fid || null)
