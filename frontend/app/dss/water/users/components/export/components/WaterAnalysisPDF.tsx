...
//       doc.text(
//         `Figure 1: Spatial distribution of water availability - ${season} ${year}-${endYear}`,
//         PW / 2, y, { align: "center" }
//       );
//       y += 8;
...
// const fileName = `WaterAvailabilityReport_${exportData.year}-${endYearFinal}_${new Date()
//   .toISOString()
//   .split("T")[0]}.pdf`;
//   doc.save(fileName);
// };
...
const { season, productType, timeScale } = exportData;
  ...
  const startYear    = rasterYears.length > 0 ? Math.min(...rasterYears) : Number(exportData.year);
 ...
 const addLogos = () => {
  ...
  const titleText    = "Comprehensive Report on Water Availability (Basin Mode)";
  const subtitleText = "A Geospatial and Hydrological Analysis for Water Resource Assessment";
...
metaLines.forEach(([label, value]) => {
  ...
  const footerMetaY = PH - 22 - 18; // just above footer band
