...
interface DrainRightPanelProps {
  isOpen: boolean;
  width: string;
  onClose: () => void;
...
export default function DrainRightPanel({
  isOpen,
  width,
  onClose,
  onWidthChange,
  panelSettings,
  isMobile = false,
}: DrainRightPanelProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const isDark = useUiModeStore((s) => s.isDark);
  const { location, chart } = useDrainViewModel();
  ...
  const filteredData = processedChartData.filter((item) => {
    const value = parseStringValue(item[selectedAttribute]);
    return value > 0;
  });
        ...
        <div className="flex h-full max-w-full flex-col" style={{ width: "100%" }}>
          <div className="flex-1 space-y-3 overflow-y-auto p-2.5 sm:space-y-4 sm:p-4">
            <div className="mb-2 flex items-center justify-between">
              <h3 className={`font-semibold ${isDark ? "text-cyan-400" : "text-blue-600"}`}>Drain Analysis</h3>
              <button
                onClick={onClose}
            ...
            {!areaConfirmed ? (
              <section className={`rounded-2xl border p-3 shadow-sm sm:p-4 ${isDark ? "border-[#1e3a5f]/50 bg-[#0d1629]/80" : "border-stone-200 bg-white/70"}`}>
                <h4 className={`mb-1.5 text-xs font-semibold sm:mb-2 sm:text-sm ${isDark ? "text-slate-300" : "text-slate-800"}`}>Analysis pending</h4>
                <p className={`text-[11px] sm:text-xs ${isDark ? "text-slate-500" : "text-slate-500"}`}>
                  Confirm stretch selections on the map to unlock advanced water quality charts and PDF reports.
                </p>
              </section>
            ) : (
              <div className="animate-fadeIn space-y-4 pb-12">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  >
                    <PdfReportBuilder
                      modeLabel="Drain"
                      dataType="stretchbased"
                      selectedIds={location.selectedStretches}
