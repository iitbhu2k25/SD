...
const WQ_PARAMETERS = [
  { key: "ph", label: "pH", unit: "", range: "6.5-8.5", min: 0, max: 14 },
  { key: "tds", label: "TDS", unit: "mg/L", range: "< 500", min: 0, max: 2000 },
  { key: "ec", label: "EC", unit: "µS/cm", range: "", min: 0, max: 5000 },
  {
    key: "temperature",
    label: "Temperature",
    unit: "°C",
  ...
  { key: "orp", label: "ORP", unit: "mV", range: "", min: -500, max: 500 },
  { key: "tss", label: "TSS", unit: "mg/L", range: "", min: 0, max: 500 },
  { key: "cod", label: "COD", unit: "mg/L", range: "< 3", min: 0, max: 100 },
  { key: "bod", label: "BOD", unit: "mg/L", range: "< 2", min: 0, max: 50 },
  {
    key: "ts",
    label: "Total Solids",
    unit: "mg/L",
...
const StretchMapComponent: React.FC<StretchChartProps> = ({ }) => {
  ...
  // Filter data for further processing
  const filteredData = useMemo(() => {
    return processedChartData.filter((item) => {
      const value = parseValue(item[selectedAttribute]);
      return value > 0;
    });
  }, [processedChartData, selectedAttribute]);
  ...
  // Create Recharts data for individual sampling locations
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      if (selectedStretches.length === 0) {
        alert("Please select stretches first");
        return;
      }
      ...
      console.log("   Stretches:", selectedStretches);
      ...
      const wsUrl = `${process.env.NEXT_PUBLIC_WEBSOCKET_DJANGO_URL}/ws/task/${jobId}`;
      ...
      ws.onmessage = (event) => {
        ...
        } else if (data.stage === "finalization_skipped") {
          console.warn("🟥 Finalization skipped:", data.message);
      ...
      const generatePDF = (result: any) => {
        ...
        // ==================== TITLE PAGE ====================
        let yPosition = 125; // Position below logos
        ...
        doc.rect(boxX, boxY, boxWidth, boxHeight); // Draw outline box
        ...
        const checkPageBreak = (heightNeeded: number) => {
          if (yPosition + heightNeeded > pageHeight - margin) {
            doc.addPage();
            addLogosToPage();
            yPosition = 10 + logoHeight; // Reset yPosition below logos
            return true;
          }
          return false;
        };
        ...
        doc.setFontSize(12);
        doc.setFont("times", "bold");
        doc.text("4.1. Stretch based", margin, yPosition);
        yPosition += 10;
        ...
        for (const resultItem of result.data.results) {
          if (resultItem.status === "success" && resultItem?.map_image) {
            ...
            checkPageBreak(20);
            // Set a box next to/below the map, or wherever you want legend
            const legendWidth = mapWidth * 0.75; // set size as needed
            const legendHeight = 16; // try 16–24 px (tune as you like)
            doc.addImage(
              resultItem.legend_image, // <-- new legend field!
              "PNG",
            ...
            // ==================== ADD WQI TABLE FOR THIS PARAMETER ====================
            checkPageBreak(30);

            const isWQI =
              paramLabel === "WATER QUALITY INDEX (WQI)" ||
              paramLabel === "WQI";
            ...
            if (isWQI) {
              ...
              // Other parameters table WITHOUT classification
        ...
        setTimeout(() => {
          setIsGeneratingPDF(false); // Auto-close overlay after 2 seconds
        }, 2000);
      };
    } catch (error: any) {
      setIsGeneratingPDF(false);
