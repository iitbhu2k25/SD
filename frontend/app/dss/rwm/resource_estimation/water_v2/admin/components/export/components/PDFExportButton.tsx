...
interface ExportRasterLike extends IndexRasterLike {
  layer_name: string;
  year?: number;
  bbox?: number[];
  workspace?: string;
...
const DEFAULT_EXPORT_BOUNDS = {
  minLon: 81.76016558413825,
  minLat: 25.25164263659975,
  maxLon: 83.05823116969096,
  maxLat: 25.790631807071463,
};

const hasRenderableLegendData = (raster: ExportRasterLike) => {
  const hasClassCounts = (raster.class_pixel_counts ?? []).some(
    (item) =>
      item.class >= 1 &&
      item.class <= 10 &&
  ...
  if (hasClassCounts) return true;

  return (raster.legend_data?.classes ?? []).some(
    (item) => item.min !== 9999 && item.max !== 9999
  );
};

const needsLegendImage = (raster: ExportRasterLike) =>
  !hasRenderableLegendData(raster);
...
const resolveExportBounds = (rasters: ExportRasterLike[]) => {
  const bbox =
    rasters.find((raster) => Array.isArray(raster.bbox) && raster.bbox.length === 4)
      ?.bbox ?? null;

  if (!bbox) {
    return {
      bbox: `${DEFAULT_EXPORT_BOUNDS.minLon},${DEFAULT_EXPORT_BOUNDS.minLat},${DEFAULT_EXPORT_BOUNDS.maxLon},${DEFAULT_EXPORT_BOUNDS.maxLat}`,
      ...DEFAULT_EXPORT_BOUNDS,
    };
  }
...
export const PDFExportButton: React.FC<PDFExportButtonProps> = ({
  ...
  const [isGenerating, setIsGenerating]       = useState(false);
  const [status, setStatus]                   = useState<"idle" | "loading" | "success" | "error">("idle");
  const hasRasterData = Boolean(exportData && rasterResponse?.clipped_rasters?.length);
  const mapFetchPromiseRef = React.useRef<Promise<{
    mapResults: { url: string; year: number }[];
    legendResults: { url: string; year: number }[];
  }> | null>(null);
  ...
  useEffect(() => {
    ...
    setIsGenerating(false);
    setStatus("idle");
    mapFetchPromiseRef.current = null;
  }, [rasterResponse]);

  const fetchAllMaps = React.useCallback(async () => {
      if (!rasterResponse?.clipped_rasters?.length) {
        return { mapResults: [], legendResults: [] };
      }

      const rasters = rasterResponse.clipped_rasters as ExportRasterLike[];
      const optimizedExpectedRasterCount = rasters.length;
      const optimizedExpectedLegendCount = rasters.filter(needsLegendImage).length;

      if (
        optimizedExpectedRasterCount > 0 &&
        mapImageUrls.length === optimizedExpectedRasterCount &&
        legendImageUrls.length === optimizedExpectedLegendCount
      ) {
        return { mapResults: mapImageUrls, legendResults: legendImageUrls };
      }

      if (mapFetchPromiseRef.current) {
        return mapFetchPromiseRef.current;
      }

      const optimizedFetchPromise = (async () => {
        setIsLoadingMap(true);

        const W = 800;
        const H = 600;
        const { bbox: BBOX, minLon, minLat, maxLon, maxLat } = resolveExportBounds(rasters);
        ...
        try {
          ...
          for (let tx = xMin; tx <= xMax; tx++) {
            for (let ty = yMin; ty <= yMax; ty++) {
              ...
              tilePromises.push(
                loadImage(tileUrl)
                  .then((img) => {
                    bgCtx.drawImage(img, px, py, px2 - px, py2 - py);
                  })
                  .catch(() => {})
              );
            }
          ...
          console.warn("Satellite tiles failed, solid background use hoga:", bgErr);
          bgCtx.fillStyle = "#c8d8c8";
          bgCtx.fillRect(0, 0, W, H);
        }
        ...
        const mapResults = await Promise.all(
          rasters.map(async (raster) => {
            const workspace =
              raster.workspace ?? process.env.NEXT_PUBLIC_FAST_RASTER_WORKSPACE;
            const wmsUrl =
              `${process.env.NEXT_PUBLIC_GEOSERVER_URL}/${process.env.NEXT_PUBLIC_FAST_RASTER_WORKSPACE}/wms?` +
              new URLSearchParams({
                service: "WMS",
                version: "1.1.0",
                request: "GetMap",
                layers: `${workspace}:${raster.layer_name}`,
                bbox: BBOX,
                width: String(W),
                height: String(H),
                srs: "EPSG:4326",
            ...
            try {
              ...
              try {
                ...
                const blob = await res.blob();
                const img2 = await loadImage(await blobToDataUrl(blob));
                ctx.globalAlpha = 0.85;
                ctx.drawImage(img2, 0, 0, W, H);
                ctx.globalAlpha = 1.0;
              } catch (fallbackError) {
                console.error(
                  `WMS load fail: ${raster.layer_name}`,
                  directLoadError,
                  fallbackError
                );
              }
              ...
              .map(async (raster) => {
                ...
                const legendUrl =
                  ...
                  new URLSearchParams({
                    ...
                    version: "1.1.0",
                    request: "GetLegendGraphic",
                    layer: `${workspace}:${raster.layer_name}`,
                    format: "image/png",
                    width: "30",
                    height: "30",
                    scale: "500000",
                ...
                try {
                  ...
                  const legBlob = await legRes.blob();
                  return {
                    url: await blobToDataUrl(legBlob),
                    year: raster.year ?? 0,
                  };
                } catch (error) {
                  console.warn(`Legend fetch fail: ${raster.layer_name}`, error);
                  return null;
                }
              })
          )
        ).filter(
          (item): item is { url: string; year: number } => item !== null
        );
        ...
        .catch((error) => {
          console.error("fetchAllMaps mein error:", error);
          return { mapResults: [], legendResults: [] };
        })
        .finally(() => {
          setIsLoadingMap(false);
      ...
      mapFetchPromiseRef.current = optimizedFetchPromise;
      return optimizedFetchPromise;

      const expectedRasterCount = rasterResponse?.clipped_rasters?.length ?? 0;
      if (expectedRasterCount > 0 && mapImageUrls.length === expectedRasterCount) {
        return { mapResults: mapImageUrls, legendResults: legendImageUrls };
      }
        ...
        for (const raster of rasters) {
          ...
          try {
            ...
            try {
              ...
              console.error(`❌ WMS Load fail: ${raster.layer_name}`, e2);
            }
          }
    ...
    [legendImageUrls, mapImageUrls, rasterResponse]
  );
  ...
  const handleDownload = () => {
    ...
    setTimeout(async () => {
      try {
        const { mapResults, legendResults } = await fetchAllMaps();
        ...
        console.log("📤 PDF ko ja raha data:", {
          mapImageUrlsCount:    mapResults.length,
          legendImageUrlsCount: legendResults.length,
        ...
        await generateWaterAnalysisPDF({
          ...
          rasterResponse,
          subdistrictCodes,
          mapImageUrls:    mapResults.length    > 0 ? mapResults    : undefined,
          legendImageUrls: legendResults.length > 0 ? legendResults : undefined,
    ...
    <button
      ...
      className={`
        ...
        ${busy
          ...
          : status === "success" ? "bg-teal-600 text-white"
          : status === "error"   ? "bg-amber-600 text-white"
