...
export async function generateWaterAnalysisPDF(
  ...
  const TITLE_FONT = 16, SUBTITLE_FONT = 11, BOX_PAD = 8;
  const titleText =
    "A Comprehensive Framework to Pixel-Level Calculation of Soil Water Content Using Space-Borne Gridded Datasets and Cloud Computing (Admin Mode)";
  const subtitleText =
    "Google Earth Engine implementation for water balance assessment in the Varuna Basin";
  ...
  //  PAGE 2 - INTRODUCTION
  // ==============================================
  newPage(); sectionHeading("1. Introduction");
  addText(
    "The Varuna Basin, situated in Uttar Pradesh, India, represents a critical hydrological unit where understanding " +
    "water balance dynamics is essential for sustainable water resource management. This project implements a " +
    "state-of-the-art workflow on Google Earth Engine, a planetary-scale geospatial platform that provides access to " +
    "petabytes of satellite imagery and climate data with high computational efficiency [1]. This report documents a " +
    "sophisticated Google Earth Engine (GEE) implementation for calculating soil water content (SWC) and water balance " +
    "dynamics across the Varuna Basin. The methodology integrates multiple Earth observation datasets - TerraCLIMATE " +
    "precipitation, evapotranspiration data from MODIS, and runoff estimates from TerraCLIMATE - to generate " +
    "comprehensive water budget assessments at pixel level (500 m resolution) and basin scale. The analysis " +
    "incorporates bias-corrected precipitation against Indian Meteorological Department (IMD) observations and " +
    "accounts for physical losses through seepage, providing hydrologically sound water availability estimates from " +
    `${startYear} to ${endYearFinal}.`
  );

  ensureSpace(30); hRule(); sectionHeading("2. Soil Water Content Formula - Theoretical Foundation");
  sectionHeading("2.1 Fundamental Water Balance Equation", 2);
  addText(
    "The water balance equation represents conservation of mass for the hydrological cycle over a defined spatial " +
    "domain and time period:"
  );
  addEquation("Delta_S = P - ET - Q - I");
  addText(
    "Where Delta_S = change in storage (soil water, groundwater), P = precipitation, ET = evapotranspiration, " +
    "Q = surface and sub-surface runoff, and I = infiltration [2]."
  );

  ensureSpace(20); sectionHeading("2.2 Relationship to GEE Water Balance Implementation", 2);
  addText(
    "The implemented water balance in Google Earth Engine adapts this framework to basin-scale satellite-based " +
    "analysis at pixel level."
  );

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 1: Correspondence between SWAT theoretical components and GEE operational implementation", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["SWAT Variable", "GEE Implementation", "Source Dataset"]],
    body: [
      ["R_day (Precipitation)", "P_corrected", "TerraCLIMATE + IMD correction"],
      ["E_a (Evapotranspiration)", "ET_normalized", "MODIS MOD16A2GF"],
      ["Q_surf + Q_gw (Total runoff)", "Q", "TerraCLIMATE runoff"],
      ["w_seep (Percolation/Seepage)", "0.12 x water budget", "[2]"],
    ],
    styles:       { font: "times", fontSize: 10, cellPadding: 4, halign: "left", valign: "middle" },
    headStyles:   { fillColor: [211, 211, 211], textColor: [0, 0, 0], fontStyle: "bold", halign: "center" },
    columnStyles: { 0: { cellWidth: 55 }, 1: { cellWidth: 40 }, 2: { cellWidth: 75 } },
  ...
  //  SECTION 3 - DATASETS
  // ==============================================
  ensureSpace(30); hRule(); sectionHeading("3. Datasets Used in Water Balance Calculation");
  sectionHeading("3.1 TerraCLIMATE Dataset", 2);
  addText(
    "TerraCLIMATE is a global high-resolution (approximately 4 km) monthly climate and climatic water balance " +
    "dataset spanning 1958 onwards [3]. The dataset combines high-resolution climatological normals from WorldClim, " +
    "time-varying data from CRU TS 4.0 and Japanese 55-year Reanalysis (JRA55), and a modified " +
    "Thornthwaite-Mather water balance model."
  );
  addText(
    "Key variables used are precipitation (pr), monthly actual evapotranspiration (aet), and runoff (ro). These " +
    "variables provide the primary depth-based inputs required for pixel-wise water balance estimation across the basin."
  );

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 2: Validation datasets for TerraCLIMATE accuracy assessment", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Validation Network", "Description"]],
    body: [
      ["Global Historical Climate Network", "Station-based precipitation and temperature"],
      ["SNOTEL Network", "Snow telemetry stations in mountain regions"],
      ["RAWS Network", "Remote Automated Weather Stations"],
      ["FLUXNET Stations", "Eddy covariance evapotranspiration observations"],
      ["Streamflow Observations", "Measured watershed runoff and discharge"],
    ],
    styles:       { font: "times", fontSize: 9.5, cellPadding: 3.5, halign: "left", valign: "middle" },
    headStyles:   { fillColor: [211, 211, 211], textColor: [0, 0, 0], fontStyle: "bold", halign: "center" },
  ...
  addText(
    "The dataset's spatial and temporal consistency makes it ideal for basin-scale water balance studies in regions " +
    "with limited ground observations [3]."
  );

  ensureSpace(20); sectionHeading("3.2 MODIS Evapotranspiration Dataset (MOD16A2GF)", 2);
  addText(
    "The MODIS Terra Net Evapotranspiration product (MOD16A2GF Version 6.1) provides gap-filled 8-day composite " +
    "evapotranspiration estimates at 500-meter spatial resolution [4]. The GF designation indicates gap-filled " +
    "processing, providing complete temporal coverage at year-end."
  );
  addText(
    "The MOD16 product is calculated using the Penman-Monteith equation, a process-based approach that accounts for:"
  );
  addEquation("ET = (Delta*(Rn-G) + rho_a*cp*(es-ea)/ra) / (Delta + gamma*(1 + rs/ra))");
  addText(
    "where Rn is net radiation, G is soil heat flux, es-ea is vapor pressure deficit, rs is surface resistance, " +
    "ra is aerodynamic resistance, Delta is the slope of the saturation vapor pressure curve, and gamma is the " +
    "psychrometric constant."
  );
  addText(
    "MOD16 incorporates MODIS vegetation indices (LAI, NDVI) for canopy characterization, MODIS albedo and land " +
    "cover classification, daily meteorological reanalysis data, and ground-based FLUXNET calibration data."
  );

  ensureSpace(20); sectionHeading("3.3 IMD Rainfall Data", 2);
  addText(
    "The Indian Meteorological Department (IMD) maintains an extensive network of rain gauge stations with daily " +
    "observation records. The implemented GEE code applies bias correction to TerraCLIMATE precipitation using " +
    "empirically derived coefficients calibrated against IMD observations. This approach reduces satellite " +
    "precipitation bias in monsoon-dominated regions where convective rainfall creates high uncertainty [5]. Bias " +
    "correction yielded a slope of 1.01 and an intercept of 10.12."
  );

  // ==============================================
  //  SECTION 4 - IMPLEMENTATION
  // ==============================================
  ensureSpace(30); hRule(); sectionHeading("4. Implementation in GEE");
  sectionHeading("4.1 Data Processing Steps", 2);
  addText(
    "The GEE implementation follows a structured approach: (1) temporal aggregation of TerraCLIMATE and MODIS data " +
    "to the selected seasonal or annual period, (2) IMD-calibrated precipitation bias correction, (3) ET " +
    "normalization, (4) spatial aggregation of runoff, precipitation, and ET across the analysis period, and (5) " +
    "pixel-by-pixel water balance computation."
  );

  ensureSpace(20); sectionHeading("4.2 Pixel Area Scaling", 2);
  addText(
    "MODIS and TerraCLIMATE data are originally expressed in mm per pixel. The Varuna Basin raster is resampled " +
    "to 500-meter resolution (approximately 0.25 km2 per pixel). Multiplying by 0.25 converts millimeter depth to " +
    "volume in mm x 0.25 km2, producing water volume estimates in units of mm.km2, or equivalently million liters."
  );

  ensureSpace(20); sectionHeading("4.3 ET Normalization Procedure", 2);
  addText(
    "MODIS and TerraCLIMATE have different calibrations and grid sizes. Direct subtraction would introduce " +
    "systematic biases. The normalization approach rescales MODIS ET to match TerraCLIMATE's range while " +
    "preserving spatial patterns [1]. This cross-sensor calibration approach follows established multi-source data " +
    "integration methodologies [6]."
  );

  ensureSpace(20); sectionHeading("4.3.1 Normalization Algorithm", 2);
  addText("Step 1: Extract min-max statistics for MODIS ET across the basin:");
  addEquation("MODIS_norm = (MODIS_ET - MODIS_min) / (MODIS_max - MODIS_min)");
  addText("For TerraCLIMATE AET across the basin:");
  addEquation("TC_range = TC_max - TC_min");
  addText("Step 2: Rescale to the TerraCLIMATE range:");
  addEquation("ET_normalized = MODIS_norm * (TC_max - TC_min) + TC_min");
  addText(
    "Result: MODIS maintains its spatial heterogeneity while conforming to TerraCLIMATE's statistical distribution."
  );

  ensureSpace(20); sectionHeading("4.4 Seepage Loss Adjustment (0.88 Factor)", 2);
  addText(
    "This factor compensates for 12% loss through seepage, representing subsurface percolation into deeper aquifers " +
    "beyond the soil root zone, along with capillary rise and redistribution within the soil profile."
  );

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 3: Water balance allocation following seepage adjustment", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Component", "Percentage", "Interpretation"]],
    body: [
      ["Available soil water content (0.88x)", "88%", "Water available in the root zone of the soil."],
      ["Seepage loss (0.12x)", "12%", "Water percolating to the vadose zone."],
    ],
    styles:       { font: "times", fontSize: 9.5, cellPadding: 3.5, halign: "left", valign: "middle" },
    headStyles:   { fillColor: [211, 211, 211], textColor: [0, 0, 0], fontStyle: "bold", halign: "center" },
    columnStyles: { 0: { cellWidth: 56 }, 1: { cellWidth: 28 }, 2: { cellWidth: 86 } },
    margin:       { left: M, right: M, top: CONTENT_TOP },
    didDrawPage:  () => addLogos(),
  });
  y = (doc as any).lastAutoTable.finalY + 10;

  ensureSpace(20); sectionHeading("4.5 Combined Water Balance Equation", 2);
  addText("Final operational formula:");
  addEquation("W_b = (P_corrected - ET_normalized - Q) * 0.25 * 0.88", { highlight: true });
  addText(
    "where P_corrected = IMD-corrected precipitation (mm), ET_normalized = MODIS ET rescaled to TerraCLIMATE " +
    "range (mm), Q = TerraCLIMATE runoff (mm), 0.25 = pixel area conversion factor (to km2), and 0.88 = seepage " +
    "loss compensation (12% loss)."
  );

  ensureSpace(20); sectionHeading("4.6 Daily Water Balance", 2);
  addEquation("W_daily = W_b / n_days");
  addText("where n_days is the number of days in the analysis period.");

  ensureSpace(20); sectionHeading("4.7 Soil Water Content Index (SWCI)", 2);
  addText("The Soil Water Content Index (SWCI) is a standardized anomaly (z-score) of daily water balance:");
  addEquation("SWCI = (WB_daily - mu_WB) / sigma_WB");
  addText(
    "where WB_daily is the daily mean water balance for a year or period, mu_WB is the long-term mean daily water " +
    "balance, and sigma_WB is its standard deviation."
  );

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 5: SWCI classes", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Class Code", "SWCI Range", "Class Name"]],
  ...
  // ==============================================
  //  SECTION 5 - SEASONAL AND OUTPUT FRAMEWORK
  // ==============================================
  ensureSpace(30); hRule(); sectionHeading("5. Seasonal and Multi-Year Analysis Framework");
  sectionHeading("5.1 Temporal Disaggregation", 2);
  addText("The GEE implementation supports analysis at multiple temporal scales [7].");

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 6: Seasonal periods defined for water balance analysis", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Season", "Period", "Hydrological Significance"]],
    body: [
      ["Winter", "Jan, Feb, Dec", "Cool, dry; very low rainfall contribution to annual total"],
      ["Pre-monsoon", "Mar-May", "Dry season, peak evapotranspiration"],
      ["Summer-monsoon", "Jun-Sept", "Primary precipitation season, high runoff"],
      ["Post-monsoon", "Oct-Nov", "Retreating monsoon; important for recharge and recession flows"],
      ["Entire year", "Jan-Dec", "Annual water balance assessment"],
    ],
    styles:       { font: "times", fontSize: 10, cellPadding: 4, halign: "left", valign: "middle" },
    headStyles:   { fillColor: [211, 211, 211], textColor: [0, 0, 0], fontStyle: "bold", halign: "center" },
    columnStyles: { 0: { cellWidth: 38 }, 1: { cellWidth: 28 }, 2: { cellWidth: 104 } },
    margin:       { left: M, right: M, top: CONTENT_TOP },
    didDrawPage:  () => addLogos(),
  });
  y = (doc as any).lastAutoTable.finalY + 10;

  ensureSpace(20); sectionHeading("5.2 Multi-Decadal Analysis", 2);
  addText(
    "Long-term water balance trends are assessed across five 5-year periods (2000-04, 2005-09, 2010-14, " +
    "2015-19, 2020-24) and one 25-year period (2000-2024)."
  );

  ensureSpace(30); hRule(); sectionHeading("6. Output Products and Classification");
  addText("The GEE application generates four primary output classes:");

  ensureSpace(12);
  doc.setFontSize(10); doc.setFont("times", "normal"); doc.setTextColor(0, 0, 0);
  doc.text("Table 7: Primary output products", PW / 2, y, { align: "center" });
  y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Product", "Definition", "Units"]],
  ...
  // ==============================================
  //  SECTION 7 - GENERATED ANALYSIS OUTPUTS
  // ==============================================
  ensureSpace(30); hRule(); sectionHeading("7. Generated Analysis Outputs");
  addText(
    "The following pages present the generated basin-scale outputs for the selected analysis, including mapped " +
    "distributions, temporal charts, and summary water budget metrics derived from the water balance workflow."
  );
  ...
  if (mapImageUrls && mapImageUrls.length > 0) {
    ...
    for (let i = 0; i < mapImageUrls.length; i++) {
      ...
      // Legend panel — always rendered (WMS image + optional class swatches for Index)
      {
        const legX = imgX + imgW + legGap, legY = imgY, legPad = 3, titleH = 9;
        // Shadow
        doc.setFillColor(200, 210, 225);
        doc.roundedRect(legX + 1.2, legY + 1.2, legW, imgH, 3, 3, "F");
        // Panel bg
        doc.setFillColor(248, 252, 255); doc.setDrawColor(22, 78, 99); doc.setLineWidth(0.5);
        doc.roundedRect(legX, legY, legW, imgH, 3, 3, "FD");
        // Title bar
        doc.setFillColor(22, 78, 99);
        doc.roundedRect(legX, legY, legW, titleH + 2, 3, 3, "F");
        doc.rect(legX, legY + titleH - 2, legW, 4, "F");
        doc.setFontSize(7.5); doc.setFont("times", "bold"); doc.setTextColor(255, 255, 255);
        doc.text("Legend", legX + legW / 2, legY + legPad + 3.5, { align: "center" });
        // Product label under title
        doc.setFontSize(6.2); doc.setFont("times", "italic"); doc.setTextColor(22, 78, 99);
        doc.text(productType, legX + legW / 2, legY + titleH + 5, { align: "center" });

        const classData: { color: string; label: string }[] = [];
        const thisRaster = clippedRasters.find((r: RasterData) => r.year === mapItem.year) ?? clippedRasters[0];

        if (isIndexProduct && thisRaster?.class_pixel_counts?.length) {
          // Index: render color swatches from class_pixel_counts
          const classes = [...(thisRaster.class_pixel_counts as any[])]
            .filter((c: any) => c.class >= 1 && c.class <= 10)
            .sort((a: any, b: any) => a.class - b.class);
          classes.forEach((c: any) => classData.push({ color: c.color, label: c.label }));
        } else if (thisRaster?.legend_data?.classes?.length) {
          // Non-Index: render from legend_data classes
          const filtered = (thisRaster.legend_data.classes as any[]).filter(
            (c: any) => c.min !== 9999 && c.max !== 9999
          );
          filtered.forEach((c: any) => classData.push({ color: c.color, label: c.label }));
        }

        if (classData.length > 0) {
          const swatchY0 = legY + titleH + 9;
          const swatchH  = (imgH - titleH - 11) / classData.length;
          const swatchSz = Math.min(swatchH * 0.65, 5);
          classData.forEach((cls, ci) => {
            const sy = swatchY0 + ci * swatchH + (swatchH - swatchSz) / 2;
            const sx = legX + legPad;
            // Color swatch
            const hexR = parseInt(cls.color.slice(1, 3), 16) || 0;
            const hexG = parseInt(cls.color.slice(3, 5), 16) || 0;
            const hexB = parseInt(cls.color.slice(5, 7), 16) || 0;
            doc.setFillColor(hexR, hexG, hexB);
            doc.setDrawColor(100, 100, 100); doc.setLineWidth(0.15);
            doc.rect(sx, sy, swatchSz, swatchSz, "FD");
            // Label
            doc.setFontSize(5.5); doc.setFont("times", "normal"); doc.setTextColor(30, 30, 30);
            const labelText = String(cls.label ?? "").substring(0, 22);
            doc.text(labelText, sx + swatchSz + 2, sy + swatchSz * 0.75);
          });
        } else if (thisLegend) {
          // Fallback: WMS legend image
          const imgAreaY = legY + titleH + 9;
          const imgAreaH = imgH - titleH - 11;
          try { doc.addImage(thisLegend, "PNG", legX + legPad, imgAreaY, legW - legPad * 2, imgAreaH); } catch (_) {}
        }
      }
  ...
  if (isIndexProduct && chartImageUrls && chartImageUrls.length > 0) {
    ...
    // Index class legend table — rendered from first raster class_pixel_counts
    const indexRaster = clippedRasters[0] as any;
    const indexClasses: any[] = (indexRaster?.class_pixel_counts ?? [])
      .filter((c: any) => c.class >= 1 && c.class <= 10)
      .sort((a: any, b: any) => a.class - b.class);

    if (indexClasses.length > 0) {
      ensureSpace(20);
      sectionHeading("SWCI Class Legend", 2);
      addText("The table below lists all Soil Water Content Index (SWCI) classes with their corresponding color, range, and label used in the spatial distribution maps and donut charts above.");

      const totalPx = indexClasses.reduce((s: number, c: any) => s + (c.pixel_count ?? 0), 0) || 1;
      autoTable(doc, {
        startY: y,
        head: [["Class", "Color", "Label", "Area (km²)", "Area (%)"]],
        body: indexClasses.map((c: any) => {
          const areaSqKm = ((c.pixel_count ?? 0) * 0.25).toFixed(2);
          const areaPct  = (((c.pixel_count ?? 0) / totalPx) * 100).toFixed(2);
          return [String(c.class), "", String(c.label ?? ""), areaSqKm, areaPct + "%"];
        }),
        styles:       { font: "times", fontSize: 9.5, cellPadding: 3, halign: "center", valign: "middle" },
        headStyles:   { fillColor: [22, 78, 99], textColor: [255, 255, 255], fontStyle: "bold", halign: "center" },
        columnStyles: { 0: { cellWidth: 18 }, 1: { cellWidth: 18 }, 2: { cellWidth: 80 }, 3: { cellWidth: 30 }, 4: { cellWidth: 24 } },
        margin:       { left: M, right: M, top: CONTENT_TOP },
        didDrawPage:  () => addLogos(),
        didDrawCell:  (data: any) => {
          // Draw color swatch in column 1
          if (data.column.index === 1 && data.section === "body") {
            const cls = indexClasses[data.row.index];
            if (cls?.color) {
              const hexR = parseInt(String(cls.color).slice(1, 3), 16) || 200;
              const hexG = parseInt(String(cls.color).slice(3, 5), 16) || 200;
              const hexB = parseInt(String(cls.color).slice(5, 7), 16) || 200;
              const sw = 8, sh = 5;
              const sx = data.cell.x + (data.cell.width - sw) / 2;
              const sy = data.cell.y + (data.cell.height - sh) / 2;
              doc.setFillColor(hexR, hexG, hexB);
  ...
  //  REFERENCES
  // ==============================================
  ensureSpace(30); hRule(); sectionHeading("References");
  const references = [
