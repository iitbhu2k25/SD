...
const Maping: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const primaryWfsUrl = `/geoserver/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=${defaultWorkspace}:${primaryLayer}&outputFormat=application/json&srsname=EPSG:3857`;
  ...
  useEffect(() => {
    ...
    const secondaryWfsUrl = `/geoserver/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=${defaultWorkspace}:${secondaryLayer}&outputFormat=application/json&srsname=EPSG:3857&CQL_FILTER=${LayerFilter} IN (${Array.isArray(LayerFilterValue)
