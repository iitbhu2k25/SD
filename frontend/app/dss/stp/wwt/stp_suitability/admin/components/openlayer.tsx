...
const Mapping: React.FC = () => {
  ...
  if (layerRef.current) {
    ...
    setFeatureCounts(prev => ({ ...prev, [type]: 0 })); // ← Add this line
  }
  
  // If no new layer specified, just return after cleanup
  if (!layer) return;
  
  const wfsUrl = `${process.env.NEXT_PUBLIC_GEOSERVER_URL}/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=${defaultWorkspace}:${layer}&outputFormat=application/json&srsname=EPSG:3857${
