...
import { downloadSuitabilityCsv } from "../../utils/downloadSuitabilityCsv";
...
interface UserBottomResultsPanelProps {
  isOpen: boolean;
  height: string;
  tableData: DataRow[];
...
export default function UserBottomResultsPanel({
  isOpen,
  height,
  tableData,
  panelSettings,
  ...
  return (
    <section
      className="relative z-20 shrink-0 overflow-hidden border-t border-stone-200 bg-[linear-gradient(180deg,#f5f1ea_0%,#f2f5f7_48%,#edf3ee_100%)] transition-[height] duration-300 ease-in-out"
      style={{
        height: isOpen ? (isMobile ? panelSettings.mobileHeightOpen : height) : panelSettings.heightClosed,
      }}
    >
      <div className="flex h-full flex-col">
        ...
        >
          <div className={`min-w-0 ${isCompactHeader ? "max-w-[34vw] sm:max-w-none" : "min-w-[8rem] flex-1"}`}>
          ...
          <div className="flex shrink-0 items-center justify-self-end gap-1.5 sm:gap-2">
            <button
              type="button"
              onClick={() => downloadSuitabilityCsv(tableData, "STP_Suitability_Drain.csv")}
