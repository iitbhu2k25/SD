...
export default function ManualOpenLayersMap() {
  ...
  useEffect(() => {
    ...
    for (const entry of multiPolygonEntries) {
      ...
      const drainFeatures = entry.drainPoints.map((dp) =>
        new Feature({ geometry: new OlPoint(fromLonLat([dp.longitude, dp.latitude])), Drain_No: dp.Drain_No })
      );
  ...
  useEffect(() => {
    resultPathLayerRef.current?.setVisible(layerVisibility.suitablePath ?? true);
  }, [layerVisibility.suitablePath]);
