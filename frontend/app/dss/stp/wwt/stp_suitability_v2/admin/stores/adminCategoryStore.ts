...
async function initializeAdminCategoryStore(set: AdminCategorySet, get: AdminCategoryGet) {
  if (get().initialized) {
    return;
  }
  ...
  try {
    ...
    set({
      error: error instanceof Error ? error.message : "Failed to initialize suitability categories",
    });
  } finally {
    set({ isLoading: false });
  }
