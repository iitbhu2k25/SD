...
interface OpenLayersWorkspaceProps {
  ...
  onSetShowLegend: (visible: boolean) => void;
...
export default function OpenLayersWorkspace({
  ...
  const layerConfigSignature = useMemo(
    () =>
      JSON.stringify(
        layerConfigs.map((layer) => ({
          id: layer.id,
          layerName: layer.layerName,
          workspace: layer.workspace ?? workspace,
          filterField: layer.filter?.filterField ?? null,
          filterValue: layer.filter?.filterValue ?? null,
          color: layer.color,
          fillColor: layer.fillColor ?? null,
          zIndex: layer.zIndex,
          visibleByDefault: layer.visibleByDefault ?? true,
          interactive: layer.interactive !== false,
          fitOnLoad: layer.fitOnLoad ?? false,
        })),
      ),
    [layerConfigs, workspace],
  );
  ...
  useEffect(() => {
    ...
    Object.values(vectorLayersRef.current).forEach((layer) => layer.changed());
  }, [showTitles]);
    ...
    layerConfigs.forEach((config) => {
      ...
      const vectorLayer = new VectorLayer({
        ...
        style: (feature, resolution) => {
          ...
          if (showTitlesRef.current && featureLabel && zoom > 8) {
            styles.push(
              new Style({
                text: new Text({
  ...
  }, [layerOpacity, onSetShowLegend, rasterLayerInfo]);
    ...
    <div className="relative flex h-full flex-col bg-gray-50">
      ...
      >
        ...
        <MapRasterSelector
          ...
          layers={rasterLayers}
          selectedLayer={selectedRasterName}
          onToggle={() => setIsRasterPanelOpen((current) => !current)}
          onSelectLayer={onSelectRasterLayer}
        />
              ...
              {rasterLayerInfo && (
                <div className="rounded-xl border border-purple-200 bg-purple-50 p-4">
                  ...
                  <input
                    ...
                    step={10}
                    value={layerOpacity}
                    onChange={(event) => onSetLayerOpacity(Number.parseInt(event.target.value, 10))}
                ...
                <button
                  ...
                  className={`flex flex-col items-center rounded-xl border p-4 transition-all duration-200 ${
                    showLegend
                      ...
                      : "border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100 text-gray-700 hover:bg-gray-200"
                  }`}
                >
                  <span className="mb-2 text-lg font-semibold">{showLegend ? "ON" : "OFF"}</span>
                  <span className="text-sm font-medium">Raster Legend</span>
                </button>
              )}
        ...
        <MapLegendOverlay
          ...
          onShowLegend={() => onSetShowLegend(true)}
          onHideLegend={() => onSetShowLegend(false)}
        />
