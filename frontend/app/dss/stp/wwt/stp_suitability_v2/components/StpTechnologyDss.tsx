...
function ClusterDistancesTable({ clusters }: { clusters: ClusterInfo[] }) {
  ...
  // Check whether clusters carry pre-computed path_layer (DSS mode from Find Suitable Area)
  const hasPrecomputedPaths = clusters.some((c) => c.path_layer);
  ...
  return (
    <div className="mt-3">
      {hasPrecomputedPaths && (
        <p className="mb-1.5 text-[10px] text-slate-400 italic px-1">
          Click a cluster label to show its road network path on the map
        </p>
      )}
      <ClusterTableShell
        title="Cluster Analysis & Approaching road network"
        clusters={clusters}
        selectedRank={selectedClusterRank}
