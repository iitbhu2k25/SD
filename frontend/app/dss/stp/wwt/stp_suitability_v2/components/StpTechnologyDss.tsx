...
function ClusterTableRows({ clusters }: { clusters: ClusterInfo[] }) {
  return (
    <>
      {clusters.map((c, i) => {
          ...
          <tr key={c.cluster_rank} className="align-top">
            ...
            <td className="w-[100px] shrink-0 border-r border-slate-100 px-3 py-2.5">
              <div className="flex flex-col items-center gap-1">
                {/* Black circle — matches map */}
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-800 text-[12px] font-bold text-white">
                  {label}
                </span>
...
function ClusterTableShell({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {/* Column legend strip */}
      <div className="flex items-center gap-3 border-b border-slate-100 bg-slate-50 px-3 py-1.5 text-[10px] font-semibold">
      ...
      {clusters.length === 0 ? (
        ...
        <table className="w-full border-collapse text-[11px]">
          ...
          <tbody className="divide-y divide-slate-100">
            <ClusterTableRows clusters={clusters} />
          </tbody>
        </table>
      )}
    </div>
...
function ClusterDistancesTable({ clusters }: { clusters: ClusterInfo[] }) {
  return (
    <div className="mt-3">
      <ClusterTableShell title="Cluster Analysis Results" clusters={clusters} />
    </div>
  );
}
...
function MultiClusterDistancesTable({ groups }: { groups: PolygonClusterGroup[] }) {
  return (
    <div className="mt-3 space-y-3">
      {groups.map((group, gi) => (
        <ClusterTableShell key={gi} title={`${group.label} — Cluster Analysis`} clusters={group.clusters} />
      ))}
    </div>
  );
}
