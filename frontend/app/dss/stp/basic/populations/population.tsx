...
const Population: React.FC<PopulationProps> = ({
    ...
    useEffect(() => {
        if (selectedMethod && selectedMethod.toLowerCase().includes('cohort')) {
            ...
            fetch(`${process.env.NEXT_PUBLIC_FAST_URL}/basic/cohort/`, {
                method: 'POST',
                headers: {
            ...
            if (selectedMethod.toLowerCase().includes('demographic')) {
                apiEndpoint = `${process.env.NEXT_PUBLIC_FAST_URL}/basic/time_series/demographic/`;
                requestBody = {
                    "start_year": null,
                    "end_year": null,
                    "year": 2025,
                ...
                apiEndpoint = `${process.env.NEXT_PUBLIC_FAST_URL}/basic/time_series/arthemitic/`;
                requestBody = {
                    "start_year": null,
                    "end_year": null,
                    "year": 2025,
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (methods.cohort) {
                ...
                cohortApiRequest = fetch(`${process.env.NEXT_PUBLIC_FAST_URL}/basic/cohort/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            if (methods.timeseries) {
                try {
                    ...
                    const timeSeriesResponse = await fetch(`${process.env.NEXT_PUBLIC_FAST_URL}/basic/time_series/arthemitic/`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
            ...
            if (methods.demographic) {
                requests.push(
                    fetch(`${process.env.NEXT_PUBLIC_FAST_URL}/basic/time_series/demographic/`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
