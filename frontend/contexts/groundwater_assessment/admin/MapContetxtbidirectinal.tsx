...
const createStatePolygonStyle = (feature: any, selectedDistricts: number[]) => {
  const districtCode = feature.get('DISTRICT_C') || feature.get('District_C') || feature.get('district_code');
  const isSelected = districtCode && selectedDistricts.includes(parseInt(districtCode));
  
  return new Style({
    fill: new Fill({ 
      color: isSelected ? 'rgba(0, 188, 212, 0.4)' : 'rgba(0, 188, 212, 0.15)' 
    }),
    stroke: new Stroke({ 
...
const createDistrictPolygonStyle = (feature: any, selectedSubDistricts: number[]) => {
  const subdistrictCode = feature.get('SUBDIS_COD') || feature.get('Subdis_Cod') || feature.get('subdistrict_code');
  const isSelected = subdistrictCode && selectedSubDistricts.includes(parseInt(subdistrictCode));
  
  return new Style({
    fill: new Fill({ 
      color: isSelected ? 'rgba(255, 192, 4, 0.5)' : 'rgba(255, 192, 4, 0.3)' 
    }),
    stroke: new Stroke({ 
      color: isSelected ? '#FFD700' : '#FFC107', 
      width: isSelected ? 5 : 3 
    })
  });
};

const createSubdistrictPolygonStyle = (feature: any, selectedSubDistricts: number[]) => {
  const subdistrictCode = feature.get('SUBDIS_COD') || feature.get('Subdis_Cod') || feature.get('subdistrict_code');
  const isSelected = subdistrictCode && selectedSubDistricts.includes(parseInt(subdistrictCode));
  
  return new Style({
    fill: new Fill({ 
      color: isSelected ? 'rgba(233, 30, 99, 0.5)' : 'rgba(233, 30, 99, 0.30)' 
    }),
    stroke: new Stroke({ 
      color: isSelected ? '#FF1744' : '#E91E63', 
...
useEffect(() => {
  ...
  const handleLocationClick = (event: any) => {
    // Skip if well add mode is active
    if (isWellAddModeActive) return;
    ...
    // Check if clicking on special layers first
    let shouldHandle = true;
    mapInstanceRef.current?.forEachFeatureAtPixel(
      pixel,
      (feature, layer) => {
        const layerName = layer?.get('name');
        if (['gsr', 'trend-villages', 'manual-wells', 'raster', 'contours'].includes(layerName)) {
          shouldHandle = false;
          return true;
        }
      },
    ...
    // Handle India layer click (select state)
    if (layerName === 'india') {
      // Try multiple property name variations
      const stateName = properties.STATE || properties.State || properties.state || 
                        properties.STATE_NAME || properties.State_Name || properties.state_name;
      
      console.log('🔍 Looking for state with name:', stateName);
      
      if (stateName && locationContext.states.length > 0) {
        // Find state by name
        const state = locationContext.states.find(s => 
          s.name.toLowerCase() === stateName.toLowerCase()
        );
        
        if (state) {
          const stateId = typeof state.id === 'string' ? parseInt(state.id) : state.id;
          console.log('✅ Found state:', state.name, 'ID:', stateId);
          locationContext.handleStateChange(stateId);
        } else {
          console.warn('⚠️ State not found in context:', stateName);
          console.log('Available states:', locationContext.states.map(s => s.name));
        }
      }
    }
    
    // Handle state layer click (select/deselect district)
    else if (layerName === 'state') {
      const districtCode = properties.DISTRICT_C || properties.District_C || properties.district_code ||
                          properties.DISTRICT_CODE || properties.District_Code;
    ...
    // Handle district layer click (select/deselect subdistrict)
    else if (layerName === 'district') {
      const subdistrictCode = properties.SUBDIS_COD || properties.Subdis_Cod || properties.subdistrict_code ||
                              properties.SUBDISTRICT_CODE || properties.Subdistrict_Code;
    ...
    // Handle village layer click (select/deselect subdistrict)
    else if (layerName === 'villages' || layerName === 'village-overlay') {
      const subdistrictCode = properties.SUBDIS_COD || properties.Subdis_Cod || properties.subdistrict_code ||
                              properties.SUBDISTRICT_CODE || properties.Subdistrict_Code;
      ...
      if (subdistrictCode) {
        ...
        if (subdistrictLayerRef.current) {
          subdistrictLayerRef.current.changed();
        }
      }
    }
  };

  mapInstanceRef.current.on('singleclick', handleLocationClick);

  return () => {
    if (mapInstanceRef.current) {
      mapInstanceRef.current.un('singleclick', handleLocationClick);
    }
  };
}, [
  isWellAddModeActive,
...
const createWFSLayer = (
  ...
  // Assign correct style function for each administrative boundary
  if (layerName === "B_district") {
    // State level (shows districts within state)
    styleFunction = (feature: any) => createStatePolygonStyle(feature, locationContext.selectedDistricts);
  } else if (layerName === "B_subdistrict") {
    // District level (shows subdistricts within district)
    styleFunction = (feature: any) => createDistrictPolygonStyle(feature, locationContext.selectedSubDistricts);
  } else if (layerName === "Village" && !isVillageOverlay) {
    // Subdistrict/Village level
    styleFunction = (feature: any) => createSubdistrictPolygonStyle(feature, locationContext.selectedSubDistricts);
  } else if (isVillageOverlay) {
    styleFunction = villageOverlayStyle;
  } else {
    styleFunction = boundaryLayerStyle;
  }
