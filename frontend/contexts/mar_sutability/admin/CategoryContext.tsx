...
export const CategoryProvider = ({ children }: CategoryProviderProps) => {
  ...
  useEffect(() => {
    const fetchConditionCategories = async () => {
      try {
        setIsLoading(true);
        const response = await api.get("/gwz_operation/get_mar_sutability_category?category=condition&all_data=true")
    ...
    const fetchConstraintCategories = async () => {
      try {
        const response = await api.get("/gwz_operation/get_mar_sutability_category?category=constraint&all_data=true") 
        if (response.status !== 200) {
          throw new Error('Failed to fetch condition categories');
        }
        const data = await response.message as Category[];
