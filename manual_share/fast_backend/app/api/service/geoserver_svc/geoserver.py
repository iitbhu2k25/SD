...
class Geoserver:
    ...
    def celery_upload_vector(self, workspace_name: str, shapefile_zip_path: str, layer_name: str) -> bool:
        """Synchronous (requests-based) vector upload — safe to call from Celery tasks
        where asyncio.run() would fail on repeated calls due to event-loop reuse issues."""
        auth = (self.username, self.password)
        headers = {"Content-type": "application/zip"}

        # Delete existing store if present (shapefile stores are 1-to-1)
        check_url = f"{self.geoserver_url}/rest/workspaces/{workspace_name}/datastores/{layer_name}"
        check_resp = requests.get(check_url, auth=auth, timeout=30)
        if check_resp.status_code == 200:
            requests.delete(f"{check_url}?recurse=true", auth=auth, timeout=30)

        upload_url = (
            f"{self.geoserver_url}/rest/workspaces/{workspace_name}"
            f"/datastores/{layer_name}/file.shp?configure=first&name={layer_name}"
        )
        with open(shapefile_zip_path, "rb") as f:
            resp = requests.put(upload_url, auth=auth, data=f, headers=headers, timeout=120)

        if resp.status_code in (200, 201):
            print(f"[celery_upload_vector] uploaded '{layer_name}' OK", flush=True)
            return True
        else:
            print(f"[celery_upload_vector] FAILED '{layer_name}': {resp.status_code} {resp.text[:200]}", flush=True)
            return False
